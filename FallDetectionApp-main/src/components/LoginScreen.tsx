...
//   const { setUserData, setUserEmail, clearUserData } = useContext(UserContext);
...
//     try {
//       const response = await axios.post('http://192.168.235.143:5000/login', {
//         email,
//         password,
//       });
...
const LoginScreen: React.FC<LoginScreenProps> = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        // Set the user's email in context
        setUserEmail(email);

        // Fetch the user's personal details
        const userDetailsResponse = await axios.get(`http://192.168.235.143:5000/api/user-details?email=${email}`);
